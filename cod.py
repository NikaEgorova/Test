...
class Player(sprite.Sprite):
 ...
 # метод "постріл" (використовуємо місце гравця, щоб створити там кулю)
    def fire(self):
        if self.position == 0:
            bullets.add(Bullet('пуля,обріз.png', self.rect.right, self.rect.centery, 30, 10, 15))
        elif self.position == 1:
            bullets.add(Bullet('пуля,обріз.png', self.rect.left, self.rect.centery, 30, 10, -15)) 
    def draw(self):
        if self.position == 0:
            window.blit(self.image,(self.rect.x,self.rect.y))
        elif self.position == 1:
...
while run:
    ...
    if not finish:
        if state=='level0':
            window.blit(image.load("фон (2).png") ,(0, 0))
            window.blit(image.load("двері 1.png") ,(600, 500))
            for e in event.get():
                if e.type == KEYDOWN:
                    if e.key == K_SPACE:
                        state = "level1"
